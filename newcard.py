...
def newcard(name, stats, pos, pfp, output, nat=None, quote=None):
    ...
    w, h = carddraw.textsize(name, font=genfont("EA", int(175/(len(name)**(1/2)))))
    carddraw.text((825-(w/2),700), name, (255,215,0), font=genfont("EA", int(175/(len(name)**(1/2)))), stroke_width=2)
    if quote:
        quote = str(quote)
        w, h = carddraw.textsize(quote, font=genfont("EA", int(275/(len(quote)**(1/2)))))
        carddraw.text((644-(w/2),800), quote, font=genfont("EA", int(275/(len(quote)**(1/2)))))
