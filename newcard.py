...
def newcard(name, stats, pos, pfp, output, nat=None, quote=None):
    ...
    if pfp != None:
        ...
        pfp = Image.open("cards/avatar-missing.png").convert("RGBA")
        pfp = pfp.resize((450,450))
        card_background.paste(pfp, (600,250), pfp)
    ...
    carddraw.text((825-(w/2),725), name, (255,215,0), font=genfont("arial", int(175/(len(name)**(1/2)))), stroke_width=2)
    if quote:
        quote = str(quote)
        w, h = carddraw.textsize(quote, font=genfont("arial", int(275/(len(quote)**(1/2)))), stroke_width=2)
        carddraw.text((644-(w/2),800), quote, font=genfont("arial", int(275/(len(quote)**(1/2)))), stroke_width=2)
